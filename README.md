...
- 当前版本：`versionName 1.6`，`versionCode 7`
...
`tvbooks/menu.txt` 使用 UTF-8 JSON，可选用于指定一级目录的类型和优先顺序：
...
- 实际菜单始终根据 `tvbooks/` 下当前存在的目录动态生成；配置中不存在的目录不会显示，未配置的新目录会自动加入并推断类型。
- 音频分类的二级菜单直接来自实际子目录，新增、改名或删除后无需同步数量即可更新。

## 遥控器操作

主界面采用三栏电视布局：左侧分类，中间内容，右侧状态。方向键移动焦点，`Right` 从分类进入内容列表，`Left` 返回分类，`OK/Enter` 打开当前内容。

主界面右侧的“同步目录”按钮只在后台快速统计各目录数量，也可以在主界面按 `Menu` 触发。同步结果会缓存到 `tvbooks/config/catalog.properties`，不参与菜单生成或目录可用性判断。普通导航直接读取真实目录结构，数量只读取缓存；未同步时不显示数量但仍可正常导航和播放。“自动间隔(秒)”用于控制自动播放等待时间。
